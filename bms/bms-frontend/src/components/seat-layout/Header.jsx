          ...
          {type === "checkout" ? (
            ...
            <div className="text-center">
              <h2 className="font-bold text-lg md:text-xl">
                {showData?.data.movie.title}
              </h2>
              <p className="text-xs text-gray-500 font-semibold">
                {dayjs(showData?.data.date, "DD-MM-YYYY").format("D MMMM YYYY")} &nbsp;
                {showData?.data.startTime} at{" "}
                {showData?.data.theatre.name +
                  ", " +
                  showData?.data.theatre.city +
                  ", " +
                  showData?.data.theatre.state}
              </p>
            </div>
          )} 
      ...
      {type !== "checkout" && (
        <>
          <div className="bg-white pt-4">
            <div className="mx-auto px-6 pb-4 flex items-center gap-4 max-w-7xl">
              <div className="text-sm text-gray-700">
                <p className="text-xs text-gray-500 font-medium">
                  {dayjs(showData?.data.date, "DD-MM-YYYY").format("ddd")}
                </p>
                <p className="text-sm font-semibold text-gray-700">
                  {dayjs(showData?.data.date, "DD-MM-YYYY").format("DD MMMM")}
                </p>
              </div>
            ...
            `}
              >
                {showData?.data.startTime}
                <p className="text-[10px] text-gray-500 -mt-1">
                  {showData?.data.audioType.toUpperCase()}
                </p>
              </button>
            </div>
          </div>
